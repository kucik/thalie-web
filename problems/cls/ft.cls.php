    ...
    function get_template ($template)
     {
    	...
    	/** Strip template comments */
			...
			$block=array("/<!--\s(BEGIN|END):\s([a-zA-Z\_0-9]*)\s-->/");
			$corrected=array("\r\n <!-- \\1: \\2 --> \r\n");
	...
	*/
	function cache_file ( $content = "" )
	{
		if (($this->USE_CACHE) && (!$this->cache_file_is_updated())) {
			...
			$fname = $fname.".ft";
			// Tendo certeza que o arquivo existe e que hĂˇ permissĂŁo de escrita primeiro.
    ...
    function parse_dynamic ($Macro,$MacroName)
    {
        ...
        if($this->$ParentTag)
        {
            ...
            while ( list ($lineNum,$lineData) = each ($DataArray) )
            {
                ...
                if("<!-- BEGIN: $Macro -->" == "$lineTest" )
                {
                    $start = true;
                    $end = false;
                    $outside = false;
                }
                if("<!-- END: $Macro -->" == "$lineTest" )
                {
                    $start = false;
                    $end = true;
                    $outside = true;
...
//  ************************************************************
    ...
    function is_assigned($tpl_var) {
    	return (isset($this->PARSEVARS["$tpl_var"])?1:0);
    }
    
    function is_defined($FileTag)
    {
    	return ($this->FILELIST["$FileTag"]?1:0);
    }
